...
### 0.4 NVMe 磁盘监控设计原则：整盘级 I/O 指标，分区级空间指标

Linux 内核 `/proc/diskstats` 中分区条目缺失时间统计字段（字段12-14），分区级的延迟、利用率、队列深度等指标**无法正确计算**。这是内核设计选择，非 bug。

| 指标类别 | 采集粒度 | 示例 | 原因 |
|---------|---------|------|------|
| I/O 性能（IOPS / 带宽 / 延迟 / 利用率 / 队列深度） | 整盘 `nvme0n1` | `psutil.disk_io_counters(perdisk=True)` 返回整盘 key | 分区缺少 `read_time`/`write_time`/`busy_time` 字段 |
| 空间使用率 | 分区/文件系统 `nvme0n1p2 → /data` | `psutil.disk_usage('/data')` | 分区是空间分配的最小单位 |

**规则：**
1. `disk_collector._build_metrics()` 中 `disk_name` 归一化为整盘名（`_base_disk_name()`），入库、存储、查询端到端一致
2. `list_disks()` 返回的磁盘列表对 NVMe 分区去重，同一整盘只出现一次
3. `monitor_service.get_disk_metrics()` 查询前用 `_normalize_disk_name()` 归一化分区名
4. 前端磁盘选择器展示整盘名，可附带该盘下分区/挂载点信息辅助定位
